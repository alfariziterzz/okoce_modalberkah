...
class LoginBasic extends Controller
{
  public function index(Request $request)
  {
    ...
    if (Auth::check()) {
      ...
      if($roles->default_page != ""){
        return redirect($roles->default_page);  
      }
  ...
  function forgot_password_send_link(Request $request)
  {
    ...
    if ($userData == null) {
      ...
      $userData->save();
      Mail::to($userData->email)->send(new SendMail($userData));
    ...
    return redirect(route('auth-login-basic'))->with([
      'success' =>
        'Please check your email, to Reset Password link <br> Silahkan check Email '.$userData->email.' untuk Link Reset Password',
    ]);
  }
